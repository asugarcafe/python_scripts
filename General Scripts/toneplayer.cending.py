...
class toneplayer():
    ...
    @staticmethod
    def play_scale_hemisync_wobble_down(scale, seconds_per_tone, wobble_start = 6, wobble_end = 6, step_down = 1):
        current_wobble = wobble_start
        for step in scale:
            if current_wobble > wobble_end:
                current_wobble = current_wobble - step_down
            note1 = datasets.note_freq_wavelength(notes,step)
            freq1 = note1[0] - (current_wobble/2)
            freq2 = note1[0] + (current_wobble/2)
            print(step + ' - ' + str(note1[0]))
            print('Hemi tone: ' + str(current_wobble))
            print("{}:{}".format(freq1,freq2))
            note1 = Note(freq1) if bit else Note(freq2)
            note2 = Note(freq2) if bit else Note(freq1)
            player.play_tones(pygame.mixer, note1, note2, seconds_per_tone, K*((seconds_per_tone**2)))
        return current_wobble
...
chakras2 = ['A2','B2','C3','C#3/Db3','D3','D#3/Eb3','E3','F#3/Gb3','G3']
chakras3 = ['A3','B3','C4','C#4/Db4','D4','D#4/Eb4','E4','F#4/Gb4','G4']
chakras4 = ['A4','B4','C5','C#5/Db5','D5','D#5/Eb5','E5','F#5/Gb5','G5']

#scale_to_use
scale_to_use = list(reversed(chakras2 + chakras3))
...
start_wobble = 13.0
final_wobble = 6.0
current_wobble = start_wobble
while True:
    current_wobble = player.play_scale_hemisync_wobble_down(scale_to_use, seconds_per_tone, current_wobble, final_wobble, .25)
